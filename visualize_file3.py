...
def draw_process_diagram(yaml_file="data/test1.yml"):
    ...
    def command_view_block(slice, type):
        add_block(
            slice[type]['id'],
    ...
    def command_pattern(slice):
        ...
        offset_x()
        command_view_block(slice, 'command')
        down_arrow(
            x, y_positions['command_view'],
            x, y_positions[slice['event']['service']]
        )
        offset_x()
        event_block(slice, slice['event'])

    # view: event -> view -> trigger
    def view_pattern(slice):
        event_block(slice, slice['event'])
        up_arrow(
            x, y_positions[slice['event']['service']],
            x, y_positions['command_view']
    ...
    def automation_pattern(slice):
        event_block(slice, slice['events'][0])
        up_arrow(
